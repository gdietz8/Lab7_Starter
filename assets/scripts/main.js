...
function initializeServiceWorker() {
  ...
  //            "Using Service Workers" will help you here)
        try {
          let registration = await navigator.serviceWorker.register("./Lab7_Starter/sw.js", {
            scope: "/",
            });
  // B4. TODO - Once the service worker has been successfully registered, console
  //            log that it was successful.
          if (registration.active) {
            console.log("Service work has been successfully registered!");
          }
        }
...
async function getRecipes() {
  ...
  //            declared above
    for (let eachURL in RECIPE_URLS){
  ...
  //            function is using the async keyword so we recommend "await"
        let fetchResponse = await fetch(RECIPE_URLS[eachURL]);
  ...
  //            "await" again
        let parsedURL = await fetchResponse.json();
        //console.log(parsedURL);
  // A8. TODO - Add the new recipe to the recipes array
        recipesArray.push(parsedURL);
